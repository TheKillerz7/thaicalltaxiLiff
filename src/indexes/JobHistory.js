...
import { getAllJobsByDriverId, getBookingByStatusWithoutDriverId, getDriverById, getJobsByDriverId } from "../apis/backend";
...
import JobHistoryList from "../pages/jobHistory/JobHistoryList";
import JobHistoryView from "../pages/jobHistory/JobHistoryView";
...
const JobHistory = () => {
  ...
  const initLine = () => {
    liff.init({ liffId: '1657246657-3pxo0X8W' }, () => {
      if (liff.isLoggedIn()) {
        runApp();
      } else {
        liff.login();
  ...
  return (
    <div>
      <div id="job" className="relative pt-16 grid w-full">
        <div className="text-3xl font-semibold text-center mb-8">Job History</div>
        {onload ? 
          ...
          : 
          <JobHistoryList onClick={handleJobViewed} data={jobList} />
        }
      </div>
      <JobHistoryView onClick={() => setJobOpen(false)} currentJobs={currentJobs} setJobOpen={setJobOpen} bookingData={jobData} isOpen={isJobOpen} userId={userId} />
    </div>
  );
}
