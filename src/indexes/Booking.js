...
const Booking = () => {
  ...
  const fillInAddress = (obj, inputEl) => {
  ...
  useEffect(() => {
    ...
    visits.forEach((visit, index) => {
      ...
      autocompleteObj.addListener("place_changed", () => {
        const placeObj = fillInAddress(autocompleteObj, visit)
    ...
    autocomplete1.addListener("place_changed", () => {
      const placeObj = fillInAddress(autocomplete1, from)
    ...
    autocomplete2.addListener("place_changed", () => {
      const placeObj = fillInAddress(autocomplete2, to)
  ...
  const onSubmit = async (data) => {
    ...
    console.log(data)
    const translated = await translations(data.bookingInfo.message.en, "th")
    data.bookingInfo.message.th = he.decode(translated.data.data.translations[0].translatedText)
    if (bookingType === "A2B") {
    ...
    <div className="grid pt-10 h-screen w-full">
        {scriptStatus === "ready" && <form onSubmit={handleSubmit(onSubmit)} className="w-full text-center">
          ...
          <FormTracker currentStep={bookingType === "A2B" ? 0 : 1} body={[
            ...
            <RentAndHire placeInfo={placeInfo} setPlaceInfo={setPlaceInfo} increment={increment} setIncrement={setIncrement} unregister={unregister} register={register} setValue={setValue} />
          ]} />
...
const AToBCourse = ({ setType, register, setValue }) => {
  ...
  return (
    <div>
      ...
      <div className="mb-3">
        <Dropdown onChange={() => {}} register={register("bookingInfo.carType")} title="Car type" options={["Any type", "Economy type", "Sedan type", "Family type", "Minibus type", "VIP Van", "VIP Car"]} setValue={setValue} />
      </div>
...
const RentAndHire = ({ unregister, register, setValue, increment, setIncrement, setPlaceInfo, placeInfo }) => {
  const [time, setTime] = useState(new Date())
  const [asap, setAsap] = useState(false)
  ...
  const addAreaHandle = (id, type, index) => {
    ...
    setAreaToVisit(temp)
    setIncrement(increment + 1)
    const placeObj = placeInfo
    delete placeObj.visits[index]
    setPlaceInfo(placeObj)
  }
      ...
      {increment && Object.keys(areaToVisit).map((area, index) => {
        return (
          <div key={area} className={"relative " + (index !== areaToVisit - 1 ? "mb-3" : "mb-2")}>
            <Textinput id="visit" onChange={() => {}} reRender={increment} register={register(`bookingInfo.visit.[${index}].place`)} title="Place info to visit" setValue={setValue} />
