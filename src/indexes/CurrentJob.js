...
import { getBookingByStatusWithoutDriverId, getCurrentJobsByDriverId, getDriverById, getJobsByDriverId } from "../apis/backend";
...
import CurrentJobList from "../pages/currentJob/CurrentJobList";
import CurrentJobView from "../pages/currentJob/CurrentJobView";
...
const CurrentJob = () => {
  ...
  const initLine = () => {
    liff.init({ liffId: '1657246657-84pVvG4y' }, () => {
      if (liff.isLoggedIn()) {
        runApp();
      } else {
        liff.login();
    ...
    <div>
      <div id="job" className="relative pt-16 grid w-full">
        ...
        {onload ? 
          ...
          : 
          jobList && <CurrentJobList onClick={handleJobViewed} data={jobList} />
        }
      </div>
      <CurrentJobView onClick={() => setJobOpen(false)} currentJobs={currentJobs} liff={liff} setJobOpen={setJobOpen} bookingData={jobData} isOpen={isJobOpen} userId={userId} />
    </div>
  );
}
