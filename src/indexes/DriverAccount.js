...
import { getDriverById, ratingDriver } from "../apis/backend";
...
const DriverAccount = () => {
  ...
  const initLine = () => {
    liff.init({ liffId: '1657246657-Z5Ea5vgm' }, () => {
      if (liff.isLoggedIn()) {
        runApp();
      } else {
        liff.login();
  ...
  useEffect(() => {
    ...
    const callback = async () => {
        const driverReq = (await getDriverById(userId)).data[0]
        setDriver(driverReq)
        setLoading(false)
    }
    callback()
  ...
  return (
    <div className="flex pt-20 h-screen w-full items-center justify-start flex-col px-5">
      <div className="text-3xl font-semibold mb-10">Driver Account</div>
      {loading ? 
          ...
          : 
          <div className="text-xl font-semibold mt-5">Driver Code: {driver.driverCode}</div>
      }
    </div>
  );
}
