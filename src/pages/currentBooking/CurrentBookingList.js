...
const CurrentBookingList = ({ onClick, data }) => {

    return (
        <div className="px-3 w-screen">
            {!data?.length && <div className="text-center pt-10 text-xl font-medium">Sorry, there's no booking right now.</div>}
                    ...
                    return (
                        <div key={index} onClick={(e) => onClick(e, booking) || null} style={{ boxShadow: "0 2px 4px 0 rgba(0, 0, 0, 0.15)" }} className="px-5 pt-3 pb-1 mb-5 rounded-lg">
                            <div className="flex items-center mb-2">
                                <div className="text-sm font-medium text-gray-600 ml-2">#{(booking.id + 300000).toString().substring(0, 3) + "-" + (booking.id + 300000).toString().substring(3)}</div>
                            </div>
