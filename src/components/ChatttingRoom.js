...
const RoomsPage = ({ userId, userType }) => {
  ...
  useEffect(() => {
    let rooms = [];
    const socket = io(
      "https://thaicalltaxibackend.herokuapp.com",
      connectionOptions
    );
      ...
      <div className="w-full py-4">
        {onload ? (
          ...
          rooms.map((room, index) => {
            ...
            var end = moment(
              room.messages.latestMessage?.createdDate || room.createdDate
            ); // another date
            var duration = moment.duration(now.diff(end));
            var days =
              duration.asDays() <= 1
                ? duration.asMinutes() <= 30
                  ? duration.humanize().includes("minutes")
                    ? duration.humanize().replace("minutes", "mins")
                    : duration.humanize().replace("minute", "mins")
                  : end.format("HH.mm")
                : duration.asDays() >= 2
              ...
              <Link key={index} to={`/chat/${userType}/room/${room.roomId}`}>
                <div className="focus:bg-blue-100 flex justify-between px-5 py-4">
                  <div className="flex w-full overflow-hidden">
                    <div
                      style={{ aspectRatio: "1" }}
                      className="bg-blue-900 rounded-full h-12 mr-4"
                    ></div>
                    <div className="overflow-hidden">
                      <div className="font-medium">
                        {room.pickupTime === "ASAP"
                          ? room.pickupDate
                          : room.pickupTime +
                            ", " +
                            room.pickupDate +
                      ...
                      <div className="flex">
                        {!messages.unreadMessages.length > 0 && (
                          ...
                          >
                            {messages?.latestMessage?.message}
                          </div>
                        )}
                        {messages.unreadMessages.length > 0 && (
                          <div
                          ...
                          >
                            {messages.unreadMessages.length > 1
                              ? messages.unreadMessages.length + " new messages"
                              : messages.unreadMessages[0].message}
                          </div>
                        )}
                        {!messages.latestMessage && (
                          <div className="text-gray-500">No message yet.</div>
                        )}
                        <div className="text-right w-max font-medium text-sm text-gray-400 pt-0.5 ml-3">
                          {days}
                        </div>
                      </div>
                    </div>
          ...
          <div className="px-5 text-center text-lg font-medium pt-10">
            Sorry there's no chat rooms yet.
          </div>
        )}
      </div>
    </div>
...
const ChatPage = ({ roomId, userType, userId }) => {
  ...
  useEffect(() => {
    ...
    const getMessage = async () => {
      ...
      const prices = (await getSelectedRegisterByBookingId(booking.bookingId))
        .data[0];
      console.log(prices);
      ...
      let pickupDateStart = "";
      if (
        booking.bookingInfo.start?.pickupDate === "ASAP" ||
        booking.bookingInfo.pickupDate === "ASAP"
      ) {
        setRoomName(`ASAP`);
      } else {
        startingDate =
          booking.bookingInfo.start?.pickupDate.split("/").reverse() ||
          booking.bookingInfo.pickupDate.split("/").reverse();
        pickupDateStart = moment(
          new Date(
            startingDate[0],
            (parseInt(startingDate[1]) - 1).toString(),
            startingDate[2]
          )
        ).format("DD MMM");
        setRoomName(
          `${
            booking.bookingInfo.start?.pickupTime ||
            booking.bookingInfo.pickupTime
          }, ${pickupDateStart}`
        );
      }
  ...
  return (
    <div className="h-full">
      <div
        style={{}}
        className="py-4 px-5 flex items-center justify-between bg-blue-50 mb-5 sticky top-0"
      >
        <div className="flex items-center w-10/12 overflow-hidden">
          <Link to={`/chat/${userType}/inbox`}>
            <div>
              <FontAwesomeIcon className="text-2xl mr-5" icon={faChevronLeft} />
            </div>
          </Link>
          <div className="items-center flex overflow-hidden">
            <div
              style={{ aspectRatio: "1" }}
              className="bg-blue-900 rounded-full h-10 mr-4"
            ></div>
            <div className="overflow-hidden pr-1">
              <div className="font-medium text-lg text-ellipsis whitespace-nowrap overflow-hidden">
                {roomName}
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-end w-2/12">
          <div
            onClick={() => setOnCheckBookingInfo((current) => !current)}
      ...
      <div className="pb-24 px-3">
        {onload ? (
          ...
          <div>
            {userType === "user" && (
              <div className="border border-gray-500 px-5 py-4 rounded-md bg-gray-50 mb-5 w-10/12 mx-auto">
                <span className="font-semibold">Driver may provide:</span>
                <br />
                1. Same car type as you choose
                <br />
                2. Similar car type
                <br />
                3. Bigger car type
            ...
            {messages.map((message, index) => {
              let messageSide =
                message.senderType === userType ? "right" : "left";
                  ...
                  case "greeting":
                    return (
                      <div className="break-normal font-medium">
                        {messageSide === "left" && (
                          <div className="mb-1">{message.translated}</div>
                        )}
                        <div
                          className={
                            messageSide === "left"
                          ...
                          {messageSide === "left" && (
                            <div className="mb-1">
                              {message.translated.split(" ").length ? (
                                ...
                                <div
                                  onClick={() =>
                                    window.location.replace(message.translated)
                                  }
                                  className={
                                    "underline inline mr-1.5 " +
                                    (messageSide === "left"
                  ...
                  >
                    {messageSide === "left" && (
                      <div className="w-8 mr-3">
                        <div className="bg-blue-900 text-white grid place-items-center rounded-full w-8 h-8">
        ...
        <div className="bg-gray-200 rounded-full py-2 w-full flex justify-between items-center">
          ...
          <div className="flex items-center w-max pr-3">
            ...
            <div
              ...
              style={{ aspectRatio: "1" }}
              className="bg-blue-900 rounded-full w-9 grid place-items-center cursor-pointer"
            >
      ...
      {bookingData.bookingInfo && (
        <>
          <BookingDetail
            driver={driver}
            prices={prices}
            onCheckBookingInfo={onCheckBookingInfo}
            setOnTransfer={setOnTransfer}
            setOnCheckBookingInfo={setOnCheckBookingInfo}
...
const BookingDetail = ({
  ...
  const dateArray =
    bookingData?.bookingInfo.start?.pickupDate.split("/").reverse() ||
    bookingData.bookingInfo.pickupDate.split("/").reverse();
  ...
  const dateArrayEnd = bookingData?.bookingInfo.end?.pickupDate
    .split("/")
    .reverse();
  const pickupDateEnd =
    dateArrayEnd &&
    moment(
      new Date(
        dateArrayEnd[0],
        (parseInt(dateArrayEnd[1]) - 1).toString(),
        dateArrayEnd[2]
      )
    ).format("DD MMM YYYY");

  const { register, setValue, handleSubmit, reset, unregister, getValues } =
    useForm({
      defaultValues: {
  ...
  return (
    <div>
      <div
        ...
        style={{ zIndex: "10" }}
        className={
          "fixed h-screen w-screen transition bg-black top-0 bg-opacity-30 left-0 grid place-items-center " +
          (datePicker !== "" ? "opacity-100" : "opacity-0 pointer-events-none")
        }
      >
      ...
      <div
        className={
          "h-screen w-full fixed top-0 left-0 bg-white transition overflow-y-scroll overflow-x-hidden duration-300 " +
          (onCheckBookingInfo ? "translate-x-0" : "translate-x-full")
        }
      >
        <div className="">
          <div
          ...
          <div>
            <div>
              <div className="px-5 pt-5 bg-white pb-3">
                <div className="">
                  <div className="text-xl text-left mb-3 font-medium">
                    <span>
                      <FontAwesomeIcon
                    ...
                    Booking Code: #
                    {(bookingData.id + 300000).toString().substring(0, 3) +
                      "-" +
                      (bookingData.id + 300000).toString().substring(3)}
                  </div>
                  <form
                    onSubmit={handleSubmit(submitHandle)}
                    className="bg-blue-50 rounded-lg py-4 px-4 mb-5 relative"
                  >
                    {!onEdit[0] && userType === "driver" && (
                      <div
                        onClick={() => setOnEdit([true, onEdit[1]])}
                        style={{ aspectRatio: "1" }}
                        className="rounded-md cursor-pointer w-min grid place-items-center bg-orange-600 p-2 absolute right-3"
                      >
                        <FontAwesomeIcon
                    ...
                    {bookingData.bookingType === "R&H" ? (
                      <div className="">
                        <div>
                          <div>
                            ...
                            {increment1 &&
                              Object.keys(area).map((place, index) => {
                                return (
                                  <div key={place} className="flex items-start">
                                    <div
                                      style={{ aspectRatio: "1" }}
                                      className="relative border-4 w-4 h-4 mt-1 rounded-full border-yellow-600 font-bold mr-2"
                                    ></div>
                                    {!onEdit[0] ? (
                                      ...
                                      <div className="flex items-center w-full">
                                        <div className="bg-white w-full my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                                          <input
                                            {...register(
                                              `bookingInfo.visit.[${index}].place.name`
                                            )}
                                            placeholder=""
                                            type="text"
                                            className="outline-none w-full"
                                        ...
                                        {Object.keys(area)?.length !== 1 && (
                                          <div
                                            ...
                                            style={{ aspectRatio: "1" }}
                                            className="bg-blue-900 h-7 rounded-md grid ml-2 place-items-center cursor-pointer"
                                          >
                                            <FontAwesomeIcon
                            ...
                            {onEdit[0] && (
                              <div className="ml-6 flex bg-blue-900 text-white text-lg font-medium w-max text-center rounded-md mb-3 mt-1">
                                <div
                                  onClick={() => addAreaHandle("", "add")}
                                  className="px-3 cursor-pointer text-sm py-1"
                                >
                                  + Add
                                </div>
                              </div>
                        ...
                        {!onEdit[0] ? (
                          <div className="font-semibold mb-3">
                            {bookingData.bookingInfo.end.place.name}
                          </div>
                        ) : (
                          <div className="bg-white my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                            <input
                              {...register("bookingInfo.end.place.name")}
                              placeholder=""
                              type="text"
                              className="outline-none w-full"
                            />
                        ...
                        <table className="">
                          <tbody>
                            <tr>
                              ...
                              {onEdit[0] && (
                                <td className="text-xl pl-2 align-middle">:</td>
                              )}
                            ...
                            <tr>
                              ...
                              <td className="align-middle pl-2">
                                {!onEdit[0] ? (
                                  bookingData.bookingInfo.start.pickupTime ===
                                  "ASAP" ? (
                                    "ASAP"
                                  ) : (
                                    bookingData.bookingInfo.start.pickupTime +
                                  ...
                                  <div className="grid gap-x-2 grid-cols-2">
                                    <div className="bg-white my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                                      <Timepicker
                                        ...
                                        setValue={setValue}
                                        prefill={
                                          bookingData.bookingInfo.start
                                            .pickupTime
                                        }
                                      />
                                      <input
                                        {...register(
                                          "bookingInfo.start.pickupTime"
                                        )}
                                        placeholder=""
                                        type="text"
                                        className="outline-none w-full"
                            ...
                            <tr>
                              ...
                              <td className="align-middle pl-2">
                                {!onEdit[0] ? (
                                  bookingData.bookingInfo.end.pickupTime +
                                  ", " +
                                  pickupDateEnd
                                ) : (
                                  <div className="grid gap-x-2 grid-cols-2">
                              ...
                              <td className="align-middle pl-2">
                                {!onEdit[0] ? (
                                  ...
                                  <div className="bg-white my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                                    <select
                                      className="w-full outline-none bg-white"
                                      {...register("bookingInfo.carType")}
                                    >
                                      <option value="Economy type">
                                        Economy type
                                      </option>
                                      <option value="Sedan type">
                                        Sedan type
                                      </option>
                              ...
                              <td className="align-middle pl-2">
                                {!onEdit[0] ? (
                                  ...
                                  <div className="grid gap-x-2 grid-cols-2">
                                    <div className="bg-white my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                                      <input
                                        {...register("bookingInfo.luggage.big")}
                                        placeholder="Big"
                                        type="number"
                                        className="outline-none w-full"
                                      />
                            ...
                            {bookingData.bookingInfo.message.en && (
                              <tr>
                                ...
                                {onEdit[0] && (
                                  <td className="text-xl pl-2 align-middle">
                                    :
                                  </td>
                                )}
                                <td className="align-middle pl-2">
                      ...
                      <div className="">
                        {!onEdit[0] ? (
                          <div className="font-semibold w-10/12">
                            {bookingData.bookingInfo.from.name}
                          </div>
                        ) : (
                          <div className="bg-white my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                            <input
                              {...register("bookingInfo.from.name")}
                              placeholder=""
                              type="text"
                              className="outline-none w-full"
                            />
                        ...
                        {!onEdit[0] ? (
                          <div className="font-semibold mb-3">
                            {bookingData.bookingInfo.to.name}
                          </div>
                        ) : (
                          <div className="bg-white my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                            <input
                              {...register("bookingInfo.to.name")}
                              placeholder=""
                              type="text"
                              className="outline-none w-full"
                            />
                        ...
                        <table className="">
                          <tbody>
                            <tr>
                              ...
                              <td className="align-middle pl-2">
                                {!onEdit[0] ? (
                                  bookingData.bookingInfo.pickupTime ===
                                  "ASAP" ? (
                                    "ASAP"
                                  ) : (
                                    bookingData.bookingInfo.pickupTime +
                                  ...
                                  <div className="grid gap-x-2 grid-cols-2">
                                    <div className="bg-white my-1 px-2 text-sm py-1 transition-all rounded-md border border-gray-400">
                                      ...
                                      />
                                      <input
                                        {...register("bookingInfo.pickupTime")}
                                        placeholder=""
                                        type="text"
                                        className="outline-none w-full"
                                        hidden
                            ...
                            {bookingData.bookingInfo.message.en && (
                              <tr>
                                ...
                                {onEdit[0] && (
                                  <td className="text-xl pl-2 align-top">:</td>
                                )}
                      ...
                      <div className="grid grid-cols-2 gap-x-3 mt-3">
                        <div
                          onClick={() => {
                            if (bookingData.bookingType === "R&H") {
                              const areaTemp = {};
                              bookingData.bookingInfo.visit.forEach(
                                (visit, index) => {
                                  areaTemp[index] = visit.place.name;
                                }
                              );
                          ...
                          className="bg-gray-200 cursor-pointer rounded-md py-2 text-center text-sm font-medium"
                        >
                          Cancel
                        </div>
                        <button
                          onClick={() => setSubmitType("bookingInfo")}
                          type="submit"
                          className="bg-blue-900 cursor-pointer text-white rounded-md py-2 text-center font-medium text-sm"
                        >
                          Submit
                        </button>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="bg-blue-50 rounded-lg relative">
                      <form
                        onSubmit={handleSubmit(submitHandle)}
                        className="border-b-2 border-gray-400 h-full w-full border-dashed py-4 px-4"
                      >
                        {!onEdit[1] && userType === "driver" && (
                          <div
                            onClick={() => setOnEdit([onEdit[0], true])}
                            style={{ aspectRatio: "1" }}
                            className="rounded-md cursor-pointer w-min grid place-items-center bg-orange-600 p-2 absolute right-3"
                          >
                            <FontAwesomeIcon
                      ...
                      <div className="absolute w-5 h-5 border bg-white border-gray-400 top-full left-0 -translate-y-1/2 -translate-x-1/2 rounded-full">
                        <div
                          style={{ left: "-2px" }}
                          className="absolute w-5 h-10 bg-white -translate-x-1/2 top-1/2 -translate-y-1/2"
                        ></div>
                      </div>
                    ...
                    <div className="bg-blue-50 rounded-md py-3 px-4 mb-5">
                      ...
                      <div className="mt-2 text-sm">
                        <span className="font-medium">*Include:</span> Gas,
                        User's Extra Orders,
                        <br />
                      ...
                      <div>
                        ...
                        {!prices.newMessage && (
                          <div className="text-sm text-red-500 mt-1">
                            *You can check this info before pickup*
                          </div>
                        )}
                                    ...
                                    {prices.newMessage && (
                                      <div>
                                        <div>
                                          ...
                                          {driver?.[0].personalInfo?.phone}
                                        </div>
                                        {driver?.[0].personalInfo?.contact.map(
                                          (item, index) => {
                                            return (
                                              <div>
                                                <span className="font-medium">
                    ...
                    {userType === "driver" && (
                      <div className=" mb-5 mt-3">
                        <div
                          onClick={() => setOnTransfer(true)}
                          className="bg-blue-900 cursor-pointer text-white rounded-md py-2 text-center font-medium"
                        >
                          Transfer Job
                        </div>
                      </div>
...
const TransferJob = ({ bookingData, onTransfer, setOnTransfer, userId }) => {
  ...
  return (
    <div
      ...
      className={
        "bg-black px-5 bg-opacity-30 mb-10 h-screen w-full grid place-items-center top-0 left-0 fixed bg-white transition duration-300 " +
        (onTransfer ? "opacity-100" : "opacity-0 pointer-events-none")
      }
    >
      <form className="bg-white rounded-md py-3 px-3 w-full">
        ...
        <div>
          <Textinput
            ...
            register={register(`driver`, { required: "Driver ID" })}
            required
            setValue={setValue}
            title="รหัสคนขับรถ"
          />
        ...
        <div className="mt-2 hidden">
          <Textinput
            onChange={() => {}}
            register={register(`newMessage`)}
            setValue={setValue}
            title="Message to new driver"
            prefill="hasMessage"
          />
        </div>
        <div className="grid grid-cols-2 gap-x-3 mt-3">
          <div
            onClick={() => setOnTransfer(false)}
            className="bg-gray-300 cursor-pointer text-gray-800 rounded-md py-2 text-center font-semibold"
          >
            Cancel
          </div>
          <button
