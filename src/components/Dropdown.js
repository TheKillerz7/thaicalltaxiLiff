...
const Dropdown = ({ between, top, register, setValue, isReset, options, title }) => {
    ...
    return(
        <div className={'relative bg-transparent px-4 pb-1 transition-all rounded-lg border-2 ' + (focus ? reTitle ? "border-blue-900" : "border-red-400" : "border-gray-300")}>
            ...
            <div style={{boxShadow: "4px 4px 30px rgba(0, 0, 0, 0.20)", maxHeight: "200px"}} className={'absolute overflow-y-scroll z-10 transition top-full bg-white translate-y-1 w-full left-0 ' + (focus ? "opacity-1" : "opacity-0 pointer-events-none")}>
