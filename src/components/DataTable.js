            ...
            {data?.length > 0 && jobs.map((job, index) => {
                ...
                if (job.bookingType === "R&H") {
                    return (
                        <div key={index} onClick={(e) => onClick(e, job) || null} style={{ boxShadow: "0 2px 4px 0 rgba(0, 0, 0, 0.15)" }} className="px-5 pt-3 pb-1 mb-5 rounded-lg">
                            <div className="flex justify-between items-center">
                                <div className="font-medium text-ellipsis whitespace-nowrap overflow-hidden w-9/12">{bookingInfo.start.place.name}</div>
                                <div className="text-gray-500 text-sm text-right w-3/12">{days}</div>
                            </div>
                            ...
                            <div className="flex flex-wrap">
                                ...
                                <div className="mb-2 py-1 px-2 font-semibold text-sm bg-gray-300 text-gray-700 rounded-md mr-2">{job.bookingInfo.carSize}</div>
                            </div>
                        </div>
                    )
                } else {
                    return (
                        <div key={index} onClick={(e) => onClick(e, job) || null} style={{ boxShadow: "0 2px 4px 0 rgba(0, 0, 0, 0.15)" }} className="px-5 pt-3 pb-1 mb-5 rounded-lg">
                            <div className="flex justify-between items-center">
                                <div className="font-medium text-ellipsis whitespace-nowrap overflow-hidden w-9/12">{bookingInfo.from.name}</div>
                                <div className="text-gray-500 text-sm text-right w-3/12">{days}</div>
                            </div>
                            <div className="-my-1"><FontAwesomeIcon className="text-blue-900 mr-2" icon={faArrowDown} /></div>
